...
import com.abdi.blogapp.Fragment.ProfileFragment
        ...
        navigationView.setOnNavigationItemSelectedListener { item ->
            when (item.itemId) {
                R.id.item_home -> {
                    ...
                    if (home != null) {
                        fragmentManager.beginTransaction().hide(fragmentManager.findFragmentByTag(ProfileFragment::class.java.simpleName)!!).commit()
                        fragmentManager.beginTransaction().show(fragmentManager.findFragmentByTag(HomeFragment::class.java.simpleName)!!).commit()
                    }
                }
                R.id.item_profile -> {
                    // Handle navigation to ProfileFragment
                    fragmentManager.beginTransaction().hide(fragmentManager.findFragmentByTag(HomeFragment::class.java.simpleName)!!).commit()
